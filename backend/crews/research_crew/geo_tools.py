...
class GetTractFipsFromAddressTool(AsyncBaseTool):
    ...
    description: str = (
        "Get the Census Tract FIPS (GEOID) for a given address. Returns a dict with 'tract_fips' or 'error'."
    )
...
class GetLongLatFromAddressTool(AsyncBaseTool):
    ...
    description: str = (
        "Get longitude and latitude for a given address using the US Census Geocoding API. Returns only a dict with longitude, latitude"
    )
...
class GetTractFieldNamesTool(AsyncBaseTool):
    ...
    description: str = (
        "Get the field names and aliases from the NRI data dictionary. Returns a list of dicts with 'field_name' and 'field_alias'."
    )
...
class GetGeoDataInput(BaseModel):
    field_names: List[str] = Field(
        ..., description="List of field names to get data for."
    )
...
class GetGeoDataTool(AsyncBaseTool):
    ...
    description: str = (
        "Get the data for a given list of field names and tract FIPS. Returns a dict of field names and their corresponding data."
    )
    args_schema: Type[BaseModel] = GetGeoDataInput

    async def run_async_code(
        self, field_names: List[str], tract_fips: str
    ) -> Dict[str, Any]:
